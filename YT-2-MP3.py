...
def download():
    ...
    if 'https://www.youtube.com/watch?' in link_search:
        ...
        selection = yt.streams.filter(only_audio=True, subtype='mp4').order_by('abr').last()
    ...
    #         download_path = video.streams.filter(only_audio=True, subtype='mp4').order_by('abr').last().download(filename=file_name,output_path = path)
    #         print('')
    #         print(f'Audio downloaded to: "{download_path}"')
    #         print('')
        ...
        selection = yt.streams.filter(only_audio=True, subtype='mp4').order_by('bitrate').last()
